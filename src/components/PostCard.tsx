...
export default function PostCard({ post }: PostCardProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'solved': return 'text-text-secondary';
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-text-primary">
                  {post.user.accountType === 'company' ? post.user.businessName : post.user.name}
                </h3>
        ...
        {post.images && post.images.length > 0 && (
          <div className="flex gap-2 mb-4">
            {post.images.slice(0, 3).map((image, index) => (
              <div key={index} className="w-20 h-20 bg-border rounded-lg overflow-hidden">
                <img 
                  src={image} 
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-4">
            {post.tags.slice(0, 3).map((tag, index) => (
              <span key={index} className="px-2 py-1 bg-surface text-text-secondary text-xs rounded">
                #{tag}
              </span>
            ))}
          </div>
