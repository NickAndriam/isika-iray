...
export default function CommunityBoard({ posts }: CommunityBoardProps) {
  ...
  const getPostIcon = (type: string) => {
    switch (type) {
      ...
      case 'expert_content': return Award;
      default: return Star;
    }
  };
  ...
  const getPostColor = (type: string) => {
    switch (type) {
      case 'tip': return 'text-primary-gold';
      case 'story': return 'text-primary-red';
      case 'expert_content': return 'text-primary-green';
      ...
      <div className="space-y-3">
        {posts.slice(0, 3).map((post, index) => {
          ...
          return (
            ...
            >
              <div className={`w-8 h-8 rounded-full bg-surface flex items-center justify-center ${color}`}>
                <Icon size={16} />
              </div>
